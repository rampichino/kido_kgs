...
function updateRoom(room: ?Room, values: Object): Room {
  ...
  // The KGS server names this flag "globalOnly" (see RoomFlags.GLOBAL_GAMES_ONLY_NAME);
  // map it onto our globalGamesOnly property.
  if ("globalOnly" in values) {
    newRoom.globalGamesOnly = values.globalOnly;
  }
  return newRoom;
}
...
export function handleRoomMessage(
  ...
  if (msg.type === "ROOM_NAMES" || msg.type === "LOGIN_SUCCESS") {
    ...
    chanMem[chanId] = { ...chanMem[chanId], stale: true };
