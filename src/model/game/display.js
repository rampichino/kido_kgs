...
export function isDirectChallenge(
  game: GameChannel,
  currentUsername: ?string
): boolean {
  if (!currentUsername || !game.name) {
    return false;
  }
  return game.name.includes("Challenge to " + currentUsername);
}
...
function compareGames(
  ...
  let aDirect = isDirectChallenge(a, currentUsername);
  let bDirect = isDirectChallenge(b, currentUsername);
  if (aDirect && !bDirect) {
    return -1;
  }
  if (!aDirect && bDirect) {
...
export function sortGames(
  ...
  games.sort((a, b) => compareGames(a, b, currentUsername));
}
