...
export function validateMove(
  ...
  // 1. Check if occupied (handled by UI, but good as safety)
  if (board[loc.y][loc.x]) {
    return "Occupied intersection";
  }

  // 2. Simulate the move
  let simBoard = copyBoardState(board);
  simBoard[loc.y][loc.x] = color;

  let ret = removeDeadStonesAround(simBoard, loc.x, loc.y);
  if (ret) {
    simBoard = ret.board;
  }

  // 3. Check for suicide
  let group = getStoneGroup(simBoard, loc.x, loc.y);
  if (group && group.liberties === 0 && ruleset !== "new_zealand") {
    return "Suicide is illegal";
  }

  // 4. Check for Ko recapture
  if (parentBoard) {
    let size = board.length;
    let isKo = true;
    for (let y = 0; y < size; y++) {
      for (let x = 0; x < size; x++) {
        // Normalize empty points before comparing: an untouched intersection
        // reads back as `undefined` (sparse board rows), while a just-captured
        // one is `null`. Without this, a genuine ko recapture (null vs
        // undefined at the ko point) looks like a different position and the
        // rule is silently skipped.
        let a = simBoard[y][x] || null;
        let b = parentBoard[y][x] || null;
        if (a !== b) {
          isKo = false;
          break;
        }
    ...
    if (isKo) {
      return "Ko recapture is illegal";
    }
  }
