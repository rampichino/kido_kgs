...
export function getGamePlayerRole(
  ...
  for (let role of Object.keys(players)) {
    if (players[(role: any)] && players[(role: any)].name === name) {
      return (role: any);
    }
  }
  return null;
...
// An editable review-like channel: a demonstration board or a review (including
// rengo reviews and games loaded from the archive, which arrive as "review").
// All of these use the same server editing model — EDIT actions, an owner, and
// an editable SGF tree — so the board treats them identically.
export function isDemoOrReview(game: GameChannel): boolean {
  return (
...
// In a demo/review the server grants editing via an "EDIT" game action naming
// the user, not via the players map. The owner and any granted editors each get
// one.
export function isGameEditor(game: GameChannel, name: string) {
  let actions = game.actions;
  if (!actions) {
    return false;
  }
  return !!actions.find((a) => a.action === "EDIT" && a.user.name === name);
}

// The demo/review owner (creator). Distinct from the editor: granting control to
// someone else moves the EDIT action away, but the owner stays the owner.
export function isGameOwner(game: GameChannel, name: string) {
  let owner = game.players && game.players.owner;
...
export function getGamePlayerOtherRole(
  ...
  // Prefer a role of the opposing color — in rengo the first non-self role can
  // be your own teammate (white/white_2), and colorless roles (owner,
  // challengeCreator) aren't opponents either.
  let ownRole = getGamePlayerRole(name, players);
  let ownColor = ownRole ? getGameRoleColor(ownRole) : null;
  let fallback: ?GameRole = null;
  for (let role of Object.keys(players)) {
    let player = players[(role: any)];
    if (!player || player.name === name) {
      continue;
    }
    let color = getGameRoleColor((role: any));
    if (ownColor && color && color !== ownColor) {
      return (role: any);
    }
    if (!fallback) {
      fallback = (role: any);
