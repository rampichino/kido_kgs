...
export function parseGameSummary(values: Object): GameSummary {
  ...
  if (typeof values.moveNum === "number") {
    summary.moveNum = values.moveNum;
  }
  return summary;
}
...
function addPropsToNode(
  ...
  // Re-parsing regenerates every comment line with a fresh id and the current
  // date. Reuse the previously parsed message for lines that already existed so
  // ids stay stable (React keys) and timestamps don't jump forward.
  let oldMsgs = tree.messages[nodeId] || [];
  let parsedMsgs = parseGameComments(newNode, commentDate).map((m, i) => {
    let old = oldMsgs[i];
    return old && old.sender === m.sender && old.body === m.body ? old : m;
  });
  tree.messages = {
    ...tree.messages,
    [nodeId]: parsedMsgs,
...
function addNodeChild(
  ...
  let node = tree.nodes[nodeId];
  // A repeated CHILD_ADDED for an existing node (e.g. a server echo of a
  // client-created demo node) must not wipe that node's props and children.
  let existingChild = tree.nodes[childNodeId];
  ...
  tree.nodes = {
    ...
    [childNodeId]: existingChild || new GameNode([], [], nodeId),
  };
}
...
function removePropsFromNode(
  ...
  // Rebuild computed state so the board reflects the removal immediately — the
  // board renders markup/stones from computedState, not the raw node props.
  // (addPropsToNode does this too; without it a PROP_REMOVED only showed after
  // the next change.)
  tree.computedState = computeGameNodeStates(tree, nodeId);
}
...
function replaceNodeProp(tree: GameTree, nodeId: number, prop: SgfProp) {
  let node = tree.nodes[nodeId];
  let newProps = node.props.map((nprop) =>
  ...
  // Rebuild computed state so the change (time, rules, markup) is visible
  // immediately, matching addPropsToNode/removePropsFromNode.
  tree.computedState = computeGameNodeStates(tree, nodeId);
}
...
function getUniqueGameTime() {
  lastGameTime = Math.max(Date.now(), lastGameTime + 1);
  return lastGameTime;
}
...
export function parseGameChannel(
  ...
  } else if (values.gameSummary && values.gameSummary.gameType) {
    newChan.type = values.gameSummary.gameType;
  }
  ...
  if (values.proposal) {
    newChan.sentProposal = values.proposal;
  }
  return newChan;
}
