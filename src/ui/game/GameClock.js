...
class TimeCountdown extends Component<TimeCountdownProps, State> {
  ...
  componentDidUpdate(prevProps: TimeCountdownProps) {
    let oldClock = prevProps.clock;
    let newClock = this.props.clock;
    if (
      this.props.nodeId === prevProps.nodeId &&
  ...
  render() {
    ...
    let timeQualifier;
    let periodTicks = null;
    let sd;
    if (periods !== undefined) {
      sd = periods === 1;
      if (periods && periods > 1) {
        // Byo-yomi: show one vertical tick per remaining period instead of "(N)".
        periodTicks = (
          <span
            className="TimeCountdown-periods"
            title={periods + " byo-yomi periods left"}>
            {Array.from({ length: periods }).map((_, i) => (
              <span key={i} className="TimeCountdown-period-tick" />
            ))}
          </span>
        );
      } else {
        timeQualifier = periods === 1 ? "SD" : "";
      }
    } else if (stones) {
    ...
    let className =
      "TimeCountdown" +
      (sd && typeof seconds === "number" && seconds < 5
